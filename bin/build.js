...
const ssr = require('../packages/ssr')
const provider = require('../packages/markdown')()
const { getFolder } = require('../packages/menu')

// README.md 替换 index.html
...
module.exports = async function (options = {
  ...
  console.log([
    `${chalk.green(`生成静态文件：`)}`,
    `源码目录: ${chalk.cyan(`${options.root}`)}`,
    `输出目录: ${chalk.cyan(`${options.output}`)}`
  ].join('\n  '))
  ...
  provider.assetsPath = filePath => path.resolve(__dirname, '../packages/', filePath)
  ...
  // 复制 asssets 文件
  console.log(chalk.yellow(`文件复制：`))
  await copyAssets('assets', provider)

  // 生成静态
  console.log(chalk.green(`生成静态：`))
